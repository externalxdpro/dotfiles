...
myBorderWidth = 2
myWorkspaces    = ["dev","www","game","sys","doc","virt","chat","mus","vid","gfx"]
...
myManageHook = composeAll . concat $
    ...
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "dev" | x <- my1Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "www" | x <- my2Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "game" | x <- my3Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "sys" | x <- my4Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "doc" | x <- my5Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "virt" | x <- my6Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "chat" | x <- my7Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "mus" | x <- my8Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "vid" | x <- my9Shifts]
    , [(className =? x <||> title =? x <||> resource =? x) --> doShiftAndGo "gfx" | x <- my10Shifts]
    ]
    where
    ...
    my3Shifts = ["Steam", "lutris", "minecraft-launcher", "badlion client", "geforce now", "Chiaki"]
    my4Shifts = ["Thunar"]
    my5Shifts = ["libreoffice"]
    my6Shifts = ["virt-manager"]
    my7Shifts = ["discord"]
    my8Shifts = ["spotify"]
    my9Shifts = ["vlc", "mpv", "obs", "kdenlive"]
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  , ((controlMask .|. shiftMask , xK_Escape ), spawn $ "xterm 'htop task manager' -e htop")
